...
#
# By default this links a LOCALLY built libduocb.xcframework
# (DUOCB_LOCAL_XCFRAMEWORK=1) so you test the Rust FFI in ../duocb as
# it stands on disk — build it first with `(cd ../duocb && ./build-ios.sh
...
Options:
  ...
  -c, --configuration NAME    Build configuration. Defaults to Debug.
      --pinned                Link the released xcframework (Package.swift default)
                              instead of a local ../duocb/dist/ios build.
...
export_local_env() {
  if [[ "$USE_LOCAL_XCFRAMEWORK" == "1" ]]; then
    local link="$PROJECT_ROOT/Packages/Duocb/local/libduocb.xcframework"
    [[ -e "$link" ]] || die "local xcframework not found at $link — build it with '(cd ../duocb && ./build-ios.sh release)'"
    export DUOCB_LOCAL_XCFRAMEWORK=1
  else
    unset DUOCB_LOCAL_XCFRAMEWORK || true
  fi
}
export_local_env
...
if [[ "$LAUNCH" == "1" ]]; then
  ...
  launch_log="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/duocb-launch.XXXXXX")"
